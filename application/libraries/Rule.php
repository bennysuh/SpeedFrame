...
class Rule{
	 ...
	 */
	public static function check($data = array() , $rule = array()){
		$index = 0;
		$success = true;
		self::$reslut = array();
		self::$reslut_other = array();
		// if(count($data) != count($rule)) return false;
		foreach ($data as $key => $value) {
			$length = @iconv_strlen($value);
			if(isset($rule[$index])){
				$this_rule = $rule[$index];
				$this_name = self::$toast_message['header'] . $this_rule['name'];

				if(isset($this_rule['is_equal']) && $value != $this_rule['is_equal']){
				...
				}else if(isset($this_rule['max']) && isset($this_rule['min']) && $length == 0){
					self::_reslutFormat($this_name . self::$toast_message['rule']['null']);
					$success = false;return false;
